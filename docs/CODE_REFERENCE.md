# Code reference for the M4 Student Kramers project

This document is the lookup dictionary for the Python implementation. It
records where each public function lives, its mathematical or computational
role, its inputs and outputs, and the analyses that call it. It is written for
code review and collaboration rather than as a Python tutorial.
...
$$
dV_t=
(-\eta V_t+aX_t^3+bX_t^2+cX_t+d)dt
+\sqrt{q(X_t,V_t)}\,dW_t.
$$
...
| `plot_m4_parameter_distributions(...)` | Empirical distributions of all eleven M4 coefficients |
...
| `python -m greenland_application.run_report_assets` | Refreshes the Git-trackable result snapshot and rebuilds report figures |

## `reporting.py`

The reporting module separates numerical evidence by optimizer generation.
It does not run expensive experiments.

| Function | Purpose |
|---|---|
| `refresh_report_snapshot()` | Copies selected current and historical tables into `docs/results/` |
| `build_report_figures()` | Rebuilds every figure referenced by the research report |

The snapshot has two evidence classes:

| Directory | Interpretation |
|---|---|
| `docs/results/current/` | Results produced with the current Cholesky M4 optimizer |
| `docs/results/development/` | Pre-Cholesky recovery, discrimination, and nested-bootstrap results retained as research history |

The development snapshot is intentionally visible to collaborators, but its
tables must not be combined with the current M4 likelihood contrast in a
formal test.
